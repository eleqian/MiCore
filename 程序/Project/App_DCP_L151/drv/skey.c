...
/*
说明：获取按键标识状态
参数：flag: 要获取的标志，可多个标志组合, 详见key_flag_t定义
返回值：是否所指定的标志都置位
*/
BOOL SKey_CheckFlagAll(skey_t *skey, skey_flag_t flag)
{
    BOOL res;
...
说明：获取按键标识状态
参数：flag: 要获取的标志，可多个标志组合, 详见key_flag_t定义
返回值：是否任意一个或多个所指定的标志置位
*/
BOOL SKey_CheckFlagAny(skey_t *skey, skey_flag_t flag)
{
    BOOL res;
